...
class OutboundResolutionError(OSError):
...
async def resolve_outbound_hostname(hostname: str, port: int) -> tuple[str, ...]:
    try:
        ...
        raise OutboundResolutionError() from exc

    addresses = {str(result[4][0]).split("%", 1)[0] for result in results if result[4]}
    if not addresses:
        raise OutboundResolutionError("dns_resolution_empty")
...
class OutboundNetworkPolicy:
    ...
    allowed_private_networks: tuple[IPv4Network | IPv6Network, ...] = field(repr=False)
    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(self, "allow_http", bool(allow_http))
        object.__setattr__(self, "allowed_ports", frozenset(int(port) for port in allowed_ports))
        object.__setattr__(
            self,
            "allowed_private_networks",
            tuple(ip_network(str(cidr), strict=False) for cidr in allowed_private_cidrs),
        )
        object.__setattr__(self, "resolver", resolver or resolve_outbound_hostname)
        object.__setattr__(
    ...
    async def resolve(
        ...
        *,
        ...
        try:
            parsed = urlsplit(normalize_outbound_url(url))
        except ValueError:
            raise OutboundPolicyError("url_invalid") from None
        scheme = parsed.scheme.lower()
        if scheme not in {"http", "https"}:
            raise OutboundPolicyError("scheme_not_allowed")
        if scheme == "http" and not self.allow_http:
            raise OutboundPolicyError("http_not_allowed")
        hostname = parsed.hostname
        if not hostname:
            raise OutboundPolicyError("hostname_invalid")
        ...
        try:
            ...
            try:
                ...
                raise OutboundResolutionError("dns_resolution_timeout") from None
        else:
        ...
        try:
            ...
            raise OutboundResolutionError("dns_resolution_invalid") from exc
        if not addresses:
            raise OutboundResolutionError("dns_resolution_empty")
