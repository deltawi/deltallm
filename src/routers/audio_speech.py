...
def _pick_deployment(request: Request, model_name: str) -> dict[str, Any]:
    ...
    for d in deployments:
        if d.get("model_info", {}).get("mode") == "audio_speech":
            return d
    raise ModelNotFoundError(message=f"No audio_speech deployment found for model '{model_name}'")
...
async def audio_speech(request: Request, payload: AudioSpeechRequest):
    ...
    try:
        ...
        callback_payload = build_standard_logging_payload(
            ...
            deployment_model=params.get("model"), request_payload=request_data, response_obj={"bytes": len(audio_bytes)},
