...
deltallm_deployment_state_metric = Gauge(
    "deltallm_deployment_state",
    "Deployment health state (0=healthy, 1=partial, 2=degraded)",
    ["deployment_id", "model"],
    registry=get_prometheus_registry(),
)

deltallm_deployment_latency_per_output_token_metric = Gauge(
    "deltallm_deployment_latency_per_output_token_ms",
    "Average latency per output token",
    ["deployment_id", "model"],
    registry=get_prometheus_registry(),
)

deltallm_deployment_active_requests_metric = Gauge(
    "deltallm_deployment_active_requests",
    "Current in-flight requests per deployment",
    ["deployment_id", "model"],
    registry=get_prometheus_registry(),
)

deltallm_deployment_cooldown_metric = Gauge(
    "deltallm_deployment_cooldown",
...
def set_deployment_latency_per_output_token(*, deployment_id: str, model: str, latency_ms: float) -> None:
    deltallm_deployment_latency_per_output_token_metric.labels(
        deployment_id=sanitize_label(deployment_id),
        model=sanitize_label(model),
...
def set_deployment_active_requests(*, deployment_id: str, model: str, count: int) -> None:
    deltallm_deployment_active_requests_metric.labels(
        deployment_id=sanitize_label(deployment_id),
        model=sanitize_label(model),
    ).set(max(0, int(count)))
...
def set_deployment_cooldown(*, deployment_id: str, model: str, cooldown: bool) -> None:
    deltallm_deployment_cooldown_metric.labels(
        deployment_id=sanitize_label(deployment_id),
        model=sanitize_label(model),
