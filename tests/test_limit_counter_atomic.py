...
@pytest.mark.asyncio
async def test_atomic_fallback_rate_limit_does_not_partially_increment():
    limiter = LimitCounter(redis_client=None, degraded_mode="fail_open")
    window_id = limiter._window_id(60)  # noqa: SLF001 - test-only inspection
    failing_key = f"key_rpm:key-1:{window_id}"
    unaffected_key = f"org_rpm:org-1:{window_id}"
    limiter._fallback_counters[failing_key] = (10**10, 1)  # noqa: SLF001 - test-only setup
    ...
    assert unaffected_key not in limiter._fallback_counters  # noqa: SLF001 - test-only assertion
    assert limiter._fallback_counters[failing_key][1] == 1  # noqa: SLF001 - test-only assertion
