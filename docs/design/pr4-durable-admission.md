...
The [reproducible measurements and raw samples](../benchmarks/pr4-admission/README.md) resolve the conditional work in issue #320:
...
- **Keep the existing admission protocol.** The 50-RPS profile has negligible lock waiting; the 200-RPS audit profiles do show contention and allocation rejection. Keep the singleton for the initial 50-RPS profile and record the higher-load limitation. A production partition/credit cutover needs reproduction on the declared deployment profile before choosing its quotas and topology, so no schema cutover or child migration PR is introduced here. The 200-RPS evidence is an explicit input to PR 10 and prevents a higher-throughput certification claim.
...
Reopen coalescing only with actual same-organization arrival traces that demonstrate useful batching after accounting for collection delay, bounded bytes/callers/tenants, cancellation and shutdown. Reopen partitions only with repeatable lock-wait/occupancy growth and an admission throughput plateau under the agreed workload. A partition proposal must then include additive durable grants whose quotas sum within the global bound, bounded rebalancing/early-full behavior, writer epochs/fencing, deterministic privacy locking, coordinated migration and rollback that retains or drains accepted work.

These decisions close PR 4's conditional design work for the measured profile. They do not close PR 9's Kubernetes deployment documentation or PR 10's sustained end-to-end, failure, stream and N−1 capacity qualification. The local admission probe excludes auth, Redis, HTTP, providers and consumer throughput; it cannot assign a supported concurrency number to a pod.
