...
At 50 RPS, p95 lock waits are at most 0.011 ms. At 200 RPS the audit singleton is under pressure, with mean estimated hold times around 4.6–4.7 ms and significant queue-lock waiting. These higher-load results are a **limit to this change**, not proof that the bottleneck has disappeared. They warrant the [partition protocol design](../../design/pr4-capacity-partitions.md) and separate inactive migration. Before activating partition writers, reproduce the contention on the declared deployment profile and apply the protocol's writer-fencing, quota, privacy and rollback gates. The 200-RPS results remain a required input to PR 10, and no 200-RPS reliability claim is made here.
