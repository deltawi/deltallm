...
Audit enqueue also avoids rewriting the capacity row for duplicate submissions and full-queue rejections that insert no records. The existing durable acceptance, required reserve, privacy and replay contracts still apply. See the [PR 4 measurements and decisions](../benchmarks/pr4-admission/README.md) for native regression coverage, the measured write reduction, and the conditional coalescing/partitioning decisions.
